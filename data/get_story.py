...
def ids_and_labels():
    ...
    cursor.execute("SELECT id, label, archived FROM story")
    records = cursor.fetchall()

    for record in records:
        story_ids_and_labels.append({"id": record[0], "label": record[1], "archived": record[2]})
